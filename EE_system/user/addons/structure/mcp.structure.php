...
use EEHarbor\Structure\FluxCapacitor\Base\Mcp;
...
class Structure_mcp extends Mcp
{
    ...
    public function __construct($switch = true)
    {
        ...
        $this->base_url = $this->flux->getBaseURL();
    ...
    public function index($message = false)
    {
        ...
        $data['action_url']            = $this->flux->moduleURL('delete');
        ...
        if (!ee()->db->field_exists('updated', 'structure')) {
            if ($data['ee_ver'] === '2') {
                $this->flux->flashData('message_error', "Structure Update Required: Please click the 'Run Module Updates' button below");
            } else {
                $this->flux->flashData('message_error', "Structure Update Required: Please click the 'Update' button next to 'Structure' below");
            }

            ee()->functions->redirect($this->flux->cpURL('addons'));
        }
        ...
        if (is_array($page_choices) && count($page_choices) > 0) {
            foreach ($page_choices as $key => $channel) {
                $vci++;
                $add_url = (string) $this->flux->cpURL('publish', 'create', array('channel_id'=>$key, 'template_id'=>$channel['template_id']));
                $add_urls[] = $add_url;
                $add_body .= '<li';
        ...
        if (empty($data['data']['page-ui'])) {
            return $this->flux->view('get_started', $data, true);
        }

        $data['foundation'] = $this->flux;

        return $this->flux->view('index', $data, true);
    }
    ...
    public function channel_settings($message = false)
    {
        ...
        $vars['action_url']        = $this->flux->moduleURL('channel_settings_submit');
        ...
        if ($page_choices && count($page_choices) == 1) {
            $vars['add_page_url'] = $this->flux->cpURL('publish', 'create', array('channel_id'=>key($page_choices)));
        } else {
            $vars['add_page_url'] = '#';
        }
        ...
        return $this->flux->view('channel_settings', $vars, true);
    }
    ...
    public function channel_settings_submit()
    {
        ...
        if (count($to_be_deleted) > 0) {
            ...
            return $this->flux->view('delete_channels_confirm', $vars, true);
        }

        $this->flux->flashData('message_success', "Channel Settings Updated");
        ee()->functions->redirect($this->flux->moduleURL('channel_settings'));
    }
    ...
    public function delete_channels()
    {
        ...
        $this->flux->flashData('message_success', "Channels Removed Successfully!");
        ee()->functions->redirect($this->flux->moduleURL('channel_settings'));
    }
    ...
    public function module_settings($message = false)
    {
        ...
        $vars = array();
        $vars['ee_ver'] = substr(APP_VER, 0, 1);
        $vars['action_url'] = $this->flux->moduleURL('module_settings_submit');
        ...
        return $this->flux->view('module_settings', $vars, true);
    }
    ...
    public function module_settings_submit()
    {
        ...
        $this->flux->flashData('message_success', "Structure Settings Updated");
        ee()->functions->redirect($this->flux->moduleURL('module_settings'));
    }
    ...
    public function validation()
    {
        ...
        $vars['ee_ver'] = substr(APP_VER, 0, 1);
        $vars['action_url'] = $this->flux->moduleURL('validation_submit');
        ...
        $vars['listing_id_fix_url'] = $this->flux->moduleURL('listing_site_id_fix');

        $vars['entries_missing_from_structure'] = $this->entries_missing_from_structure();

        $vars['foundation'] = $this->flux;
        $vars['themes_dir'] = $this->flux->getAddonThemesDir();
        ...
        //
        // -------------------------------------------
        return $this->flux->view('validation', $vars, true);
    }
    ...
    public function restore()
    {
        ...
        if (is_numeric($id)) {
            ...
            if ($nav_to_rollback_to->num_rows == 1) {
                ...
                $this->flux->flashData('message_success', "Updated!");
                ee()->functions->redirect($this->flux->moduleURL('nav_history'));
            }
        }
    }
    ...
    public function validation_submit()
    {
        ...
        ee()->functions->redirect($this->flux->moduleURL('validation'));
    }
        ...
        if (!empty($channels_result)) {
            ...
            if (!empty($missing_entries)) {
                foreach ($missing_entries as $missing_entry) {
                    $missing[] = array(
                        'ee_url' => $this->flux->cpURL('publish', 'edit', array('entry_id'=>$missing_entry->entry_id)),
    ...
    public function hook_change_27()
    {
        ...
        if (version_compare('2.7', APP_VER, ">")) {
            ...
            $this->flux->flashData('message_failure', lang('not_ee27'));
            ...
            $this->flux->flashData('message_success', lang('ee27_hook_complete'));
