...
use EEHarbor\Structure\FluxCapacitor\Conduit\StaticCache;
use EEHarbor\Structure\FluxCapacitor\Conduit\PersistentCache;
...
class Sql_structure
{
    ...
    public function __construct()
    {
        ...
        // echo '<div style="border:1px solid #ccc;background-color:#ffffe6;padding:5px 10px;color:#000;">StaticCache Starting:<pre>';
        // var_dump(StaticCache::all());
        // echo '</pre></div>';

        // StaticCache::set('testing', 'Yup, this is my test alright!');
        // StaticCache::set('to_delete', 'This one should be deleted next!');

        // echo '<div style="border:1px solid #ccc;background-color:#ffffe6;padding:5px 10px;color:#000;">StaticCache Additions:<pre>';
        // var_dump(StaticCache::all());
        // echo '</pre></div>';

        // if (StaticCache::has('testing')) {
        //     echo 'HAS STATIC<br />';
        // }

        // StaticCache::delete('to_delete');

        // echo '<div style="border:1px solid #ccc;background-color:#ffffe6;padding:5px 10px;color:#000;">StaticCache Deleted to_delete:<pre>';
        // var_dump(StaticCache::all());
        // echo '</pre></div>';

        // if (PersistentCache::has('persistent')) {
        //     echo 'HAS PERSISTANCE<br />';
        // }

        // echo '<div style="border:1px solid #ccc;background-color:#ffffe6;padding:5px 10px;color:#000;">PersistentCache Get:<pre>';
        // var_dump(PersistentCache::get('persistent'));
        // echo '</pre></div>';

        // if (!PersistentCache::has('persistent')) {
        //     echo 'Setting Persistent<br />';
        //     PersistentCache::set('persistent', 'Yup it sure does!');
        // }

        // echo '<div style="border:1px solid #ccc;background-color:#ffffe6;padding:5px 10px;color:#000;">PersistentCache Set:<pre>';
        // var_dump(StaticCache::all());
        // echo '</pre></div>';

        // PersistentCache::delete('persistent');

        // echo '<div style="border:1px solid #ccc;background-color:#ffffe6;padding:5px 10px;color:#000;">PersistentCache Delete Key:<pre>';
        // var_dump(StaticCache::all());
        // echo '</pre></div>';

        // PersistentCache::set('another_persistent', 'Yet another persistent!');
        // PersistentCache::set('moar_persistent', 'MOAR PERSISTANCE!');

        // echo '<div style="border:1px solid #ccc;background-color:#ffffe6;padding:5px 10px;color:#000;">StaticCache has new PersistentCache Items:<pre>';
        // var_dump(StaticCache::all());
        // echo '</pre></div>';

        // PersistentCache::clear();

        // echo '<div style="border:1px solid #ccc;background-color:#ffffe6;padding:5px 10px;color:#000;">PersistentCache Cleared:<pre>';
        // var_dump(StaticCache::all());
        // echo '</pre></div>';

        // StaticCache::clear();

        // echo '<div style="border:1px solid #ccc;background-color:#ffffe6;padding:5px 10px;color:#000;">StaticCache Cleared:<pre>';
        // var_dump(StaticCache::all());
        // echo '</pre></div>';
    ...
    public function get_settings()
    {
        ...
        if (is_array($settings) || !$this->flux->module_installed('structure')) {
            return $settings;
        }
    ...
    private function _get_sql_fields($custom_titles, $title_fields)
    {
        ...
        ee()->load->library('api');
        $this->flux->instantiate('channel_fields');
    ...
    public function get_listing_entry_ids()
    {
        ...
        // $listing_ids = $this->flux->cache('get', 'listing_ids');
        // $listing_ids_empty = $this->flux->cache('get', 'listing_ids_empty');
        $listing_ids = StaticCache::get('listing_ids');
        $listing_ids_empty = StaticCache::get('listing_ids_empty');
        // echo '<div style="border:1px solid #ccc;background-color:#ffffe6;padding:5px 10px;color:#000;">VARDUMP:<pre>';
        // var_dump($listing_ids_empty);
        // echo '</pre></div>';
        ...
        if (empty($listing_ids_empty) && (!is_array($listing_ids) || empty($listing_ids))) {
            ...
            // $this->flux->cache('set', 'listing_ids', $listing_ids);
                // $this->flux->cache('set', 'listing_ids_empty', 'false');
            } else {
                StaticCache::set('listing_ids_empty', 'true');
                // $this->flux->cache('set', 'listing_ids_empty', 'true');
            }
        }
    ...
    {
        $module_id_query = $this->flux->cache('get', 'module_id_query');

        if (! $module_id_query) {
            $module_id_query = ee()->db->query("SELECT module_id FROM exp_modules WHERE module_name = 'Structure'")->result();

            $this->flux->cache('set', 'module_id_query', $module_id_query);
        }
    ...
    public function cleanup_check()
    {
        ...
        if (count($site_pages_uri_duplicates) > 0) {
            ...
            foreach ($site_pages_uri_duplicates as $entry_id => $uri) {
                ...
                $vals['site_pages_uri_duplicates'][$entry_id]['ee_url'] = $this->flux->cpURL('publish', 'edit', array('entry_id'=>$entry_id));
            }
        }
        ...
        foreach ($structure_entries as $structure_entry) {
            ...
            if (!empty($ee_exists) && $ee_exists == 1) {
                ...
                $vals['orphaned_entries'][$structure_entry['entry_id']]['ee_url'] = $this->flux->cpURL('publish', 'edit', array('entry_id'=>$structure_entry['entry_id']));
            } else {
                $eeOrphans++;
            }
            ...
            if (in_array($structure_entry['entry_id'], $site_pages_entry_ids)) {
                ...
                if ($structure_entry['structure_url_title'] != $check_url) {
                    ...
                    $vals['mismatch_url_entries'][$structure_entry['entry_id']]['ee_url'] = $this->flux->cpURL('publish', 'edit', array('entry_id'=>$structure_entry['entry_id']));
                }
                ...
                if (!$this->is_valid_template($structure_entry['template_id'])) {
                    ...
                    $vals['template_id_errors'][$structure_entry['entry_id']]['ee_url'] = $this->flux->cpURL('publish', 'edit', array('entry_id'=>$structure_entry['entry_id']));
                }
            ...
            if (!empty($structure_entry['listing_cid']) && !in_array($structure_entry['listing_cid'], $checkedListingChannels)) {
                ...
                foreach ($listing_entries as $listing_entry) {
                    ...
                    if (!empty($listing_exists) && $listing_exists == 1) {
                        ...
                        if ($listingSpExists) {
                            ...
                            if ($listing_data['uri'] != $check_url) {
                                ...
                                $vals['mismatch_url_entries'][$listing_entry['entry_id']]['ee_url'] = $this->flux->cpURL('publish', 'edit', array('entry_id'=>$listing_entry['entry_id']));
                            }
                            ...
                            if (!$this->is_valid_template($listing_data['template_id'])) {
                                ...
                                $vals['template_id_errors'][$listing_entry['entry_id']]['ee_url'] = $this->flux->cpURL('publish', 'edit', array('entry_id'=>$listing_entry['entry_id']));
                            }
                        }
                    } else {
                    ...
                    if (!$listingSpExists || !$listingStructureExists) {
                        ...
                        $vals['orphaned_entries'][$listing_entry['entry_id']]['ee_url'] = $this->flux->cpURL('publish', 'edit', array('entry_id'=>$listing_entry['entry_id']));
                    }
                }
            }
        ...
        if ($structureOrphans > 0) {
            ...
            foreach ($site_pages_remaining as $entry_id => $url_title) {
                ...
                $vals['orphaned_entries'][$entry_id]['ee_url'] = $this->flux->cpURL('publish', 'edit', array('entry_id'=>$entry_id));
