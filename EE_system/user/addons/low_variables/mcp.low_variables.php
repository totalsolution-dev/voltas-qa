...
class Low_variables_mcp extends Low_variables_base {
	...
	public function __construct()
	{
		...
		// Set shortcuts
		$this->settings =& ee()->low_variables_settings;
		$this->types    =& ee()->low_variables_types;
		$this->sync     =& ee()->low_variables_sync;
		$this->vars     =& ee()->low_variables_variable_model;
		$this->groups   =& ee()->low_variables_group_model;
	}
		...
		if ($vars)
		{
			...
			foreach ($vars as $var)
			{
				...
				$buttons = array(array(
					...
					'type' => 'submit',
					'value' => '',
					'text' => 'low_variables_save',
					'working' => 'btn_saving'
				));

				// Add button for clearing cache
				$clear = ee()->low_variables_settings->get('clear_cache');
			...
			if ($labels)
			{
				ee('CP/Alert')->makeInline('shared-form')
					...
					->addToBody(lang('low_variables_saved_except').'<ul>'.implode("\n", $labels).'</ul>')
					->now();
			}
			...
			ee()->javascript->set_global('LOW.group_id', $group_id);
			ee()->javascript->set_global('LOW.edit_var_url', $this->mcp_url('edit_var/%d'));
	...
	public function save($group_id)
	{
		...
		$vars = low_associate_results($vars, $this->vars->pk());
		...
		$types = low_flatten_results($vars, 'variable_type');
		$types[] = Low_variables_types::DEFAULT_TYPE;
		...
		if (empty($skipped))
		{
			ee('CP/Alert')->makeInline('shared-form')
				...
				->addToBody(lang('low_variables_saved'))
				->defer();
		}
		else
		{
	...
	public function vars()
	{
		...
		// Optional columns
		if ($early) $cols['early_parsing_th'] = array('encode' => FALSE);
		...
		foreach ($vars as $var)
		{
			...
			$type = array_key_exists($var['variable_type'], $types)
				? $var['variable_type']
				: Low_variables_types::DEFAULT_TYPE;
			...
			// Early
			if ($early) $row[] = LVUI::onoff($this->mcp_url('toggle/early_parsing/'.$id), $var['early_parsing']);
	...
	public function toggle($type, $var_id)
	{
		if (ee('Request')->method() == 'POST')
		{
			...
			if (ee('Request')->isAjax()) die(json_encode($result));
		}
	...
	public function save_list()
	{
		...
		if ($vars = ee('Request')->post('variable_id'))
		{
			...
			if ($action == 'delete')
			{
				...
				$this->delete($vars);
				$msg = 'low_variables_deleted';
			}
			// elseif ($action == 'sync')
			// {
			// 	// Manual sync to file
			// 	$this->sync->files($vars);
			// 	$msg = 'low_variables_synced';
			// }
	...
	public function delete($vars = NULL)
	{
		...
		ee('CP/Alert')->makeInline('shared-form')
			->asSuccess()
			->withTitle(lang('low_variables_deleted'))
			->defer();
	...
	public function edit_var($var_id = 'new', $clone_id = NULL)
	{
		...
		// Get all groups
		$choices = low_flatten_results($this->get_groups(), 'group_label', 'group_id');
		...
		$sections[0][] = array(
			...
			'fields' => array(
				'variable_type' => array(
					'type' => 'select',
					'value' => $type,
					'choices' => low_flatten_results($types, 'name', 'type'),
					'group_toggle' => $groups
				)
			)
		);
		...
		if ($var_id == 'new')
		{
			...
			foreach ($vars as $sfx => $name)
			{
				...
				if ( ! empty($sfx) && $var['variable_label'])
				{
					...
					if (strpos($label, '%s') === FALSE) $label .= ' (%s)';
	...
	public function save_group()
	{
		...
		// Force integer
		if ($group_id != 'new') $group_id = (int) $group_id;
	...
	private function duplicate_vars_to_group($var_ids, $group_id, $suffix)
	{
		...
		$types = low_flatten_results($rows, 'variable_type');
		$types[] = Low_variables_types::DEFAULT_TYPE;
		...
		foreach ($rows as $row)
		{
			...
			// Skip existing ones
			if ($this->vars->var_exists($new_name, $row['variable_id'])) continue;
	...
	public function delete_group()
	{
		...
		if ($group_id = ee('Request')->post('id'))
		{
			...
			ee('CP/Alert')->makeInline('shared-form')
				->asSuccess()
				->withTitle(lang('low_variable_group_deleted'))
				//->addToBody(lang('group_saved_changes'))
				->defer();
		}
	...
	public function save_group_order()
	{
		...
		if ( ! ee('Request')->isAjax()) return;

		// And Groups must be given
		if ( ! ($groups = ee('Request')->post('groups'))) return;
	...
	public function settings()
	{
		...
		$key = $this->settings->get('license_key');
		...
		$sections[0][] = array(
			...
			'fields' => array(
				'license_key' => array(
					...
					'type'     => 'text',
					'value'    => $key,
					'disabled' => ee()->low_variables_settings->is_config('license_key'),
					'note'     => ($key && ! $this->valid_license()) ? lang('invalid_license') : NULL
				)
			)
		);
		...
		// 			'disabled' => ee()->low_variables_settings->is_config('save_as_files')
		// 		)
		// 	)
		// );
		...
		// 			'disabled' => ee()->low_variables_settings->is_config('one_way_sync')
		// 		)
		// 	)
		// );
		...
		// 			'disabled' => ee()->low_variables_settings->is_config('file_path')
		// 		)
		// 	)
		// );
		...
		$value[] = Low_variables_types::DEFAULT_TYPE;
		...
		$sections[0][] = array(
			...
			'fields' => array(
				'enabled_types' => array(
					...
					'choices' => $choices,
					'value' => $value,
					'disabled_choices' => array(Low_variables_types::DEFAULT_TYPE)
				)
			)
		);
	...
	public function save_settings()
	{
		...
		if ( ! in_array(Low_variables_types::DEFAULT_TYPE, $settings['enabled_types']))
		{
			$settings['enabled_types'][] = Low_variables_types::DEFAULT_TYPE;
		}
	 ...
	 * @return     bool
	 */
	private function valid_license()
	{
		$is_valid = FALSE;
	...
	private function get_groups()
	{
		...
		if (is_array($counts) && empty($counts))
		{
			...
			$groups = low_associate_results($groups, 'group_id');
		...
		ee()->javascript->set_global('LOW.save_group_order_url', $this->mcp_url('save_group_order'));
	...
	private function view($file)
	{
		...
		// Make active?
		if ($this->active == 'groups') $header->isActive();
		...
		if ($groups = $this->get_groups())
		{
			...
			foreach ($groups as $key => $val)
			{
				...
				// Is the item active?
				if (is_numeric($this->active) && is_numeric($key) && $this->active == $key) $item->isActive();
			}
		}
		...
		if ($manager)
		{
			...
			if ($this->active == 'vars') $header->isActive();
		...
		ee()->javascript->set_global('LOW.iconFont',
			version_compare(APP_VER, '3.3.0', '>=') ? 'fontawesome' : 'glyphicons'
		);

		ee()->javascript->set_global('LOW.EE4', ee4());
