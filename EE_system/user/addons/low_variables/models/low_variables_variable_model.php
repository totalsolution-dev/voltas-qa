...
{
	require_once(PATH_THIRD.'low_variables/models/low_variables_model.php');
}

/**
 * Low Variables Variable Model class
 *
 ...
 */
class Low_variables_variable_model extends Low_variables_model {
	...
	public function get_group_count($include_hidden = TRUE)
	{
		...
		return low_flatten_results($query->result_array(), 'var_count', 'group_id');
	}
	...
	public function get_file_vars($ids = array())
	{
		...
		if ($ids) ee()->db->where_in($this->prefix($this->pk(), $this->table()), $ids);
	...
	public function get_ft($group_ids = array())
	{
		...
		if ($group_ids) ee()->db->where_in('group_id', $group_ids);
...
/* End of file Low_variables_variable_model.php */