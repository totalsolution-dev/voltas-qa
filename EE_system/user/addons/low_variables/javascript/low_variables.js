...
LOW.FolderList = function() {
	...
	$list.find('.remove a.m-link').on('click', function(event){
		event.preventDefault();
...
LOW.EditLinks = function() {
	...
	$els.each(function(){
		...
		// Create edit link
		$edit.addClass('ico settings').attr('href', href);
		...
		if (EE.LOW.EE4) {
			...
			// Add edit link and code to label (h3)
			$el.find('.setting-txt :first-child')
				.wrapInner('<span/>')
				.append($edit)
				.append($code);
		}
